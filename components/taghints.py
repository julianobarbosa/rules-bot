...
HINTS = {
    '#inline': {
        ...
        'buttons': [
            {'text': '🔎 Try it out', 'switch_inline_query': '{query}'}
        ],
        ...
        'buttons': [
            {
                ...
                'url': f"https://t.me/{const.SELF_BOT_NAME}",
            }
        ],
...
def list_available_hints(update: Update, context: CallbackContext):
    message = (
        "You can use the following hashtags to guide new members:\n\n"
        + '\n'.join(
            '🗣 {tag} ➖ {help}'.format(tag=k, help=v['help'])
